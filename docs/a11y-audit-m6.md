...
| Add / Edit Transaction        | ✅                              | ✅             | ✅                | `WS12` (Add) + `WS13` (Edit) in `transaction_form_screen_test.dart` — pump at 2× scale, no exceptions, AppBar title + Save still rendered. `WS14` focuses the memo field with simulated keyboard insets and verifies the fixed keypad does not move. Calculator keypad's backspace/clear use `tooltip: l10n.txKeypadBackspace/Clear`. Edit-mode Delete in AppBar uses `tooltip: l10n.commonDelete`.                                                                                                        |
...
- **Add/Edit fixed-keypad keyboard coverage.** Added `transaction_form_screen_test.dart` `WS14`, which simulates soft-keyboard insets while focusing the memo field and asserts `CalculatorKeypad` keeps the same screen bounds. This closes Unit 8's keyboard-cover test requirement.
