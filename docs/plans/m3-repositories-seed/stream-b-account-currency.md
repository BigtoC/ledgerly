...
abstract class CurrencyRepository {
  ...
  ///
  /// [upsert] does NOT touch `custom_name` on an existing row — only
  /// the seeded / Phase 2 fields (`decimals`, `symbol`, `nameL10nKey`,
  /// `isToken`, `sortOrder`) are rewritten. User renames flow through
  /// [updateCustomName].
  Future<void> upsert(Currency currency);

  /// Writes `currencies.custom_name` for an existing row, leaving every
  /// other column untouched. Mirrors the category / account-type rename
  /// pattern so locale changes do not duplicate or orphan seeded rows.
  ///
  /// - `customName == null` clears the override; M5 renders
  ///   `AppLocalizations.of(context).currencyName(nameL10nKey)`.
  /// - Empty / whitespace-only strings are normalized to `null`.
  /// - Throws [CurrencyNotFoundException] when `code` has no row.
  ///
  /// Source of truth for the feature: §12 Q7 resolution (2026-04-22) on
  /// Stream C's plan, which pinned every seeded currency's
  /// `nameL10nKey = 'currency.<code>'` and promised user-facing rename
  /// support. Guardrail G7 (stable `name_l10n_key` identity).
...
abstract class AccountTypeRepository {
  ...
  /// parameters — not a `AccountType` value object — because the seed
  /// supplies raw seeded-row fields and intentionally never surfaces
  /// `customName` / `isArchived` / `id` to this method. Internally the
  /// repository builds the Drift companion from these fields; the domain
  /// model is never passed through the seed seam.
  ///
...
- [ ] Implement `upsertSeeded({required String l10nKey, required String icon, required int color, required Currency defaultCurrency, required int sortOrder})` per §1.2 — named-param signature frozen by Stream C §7.1 / §12 Q5 (2026-04-22). Locate the existing row via `AccountTypeDao.findByL10nKey(l10nKey)`; insert when absent, update the seeded fields when present while preserving any stored `customName` / `isArchived`. Pre-check `defaultCurrency.code` exists in `currencies` via `CurrencyRepository.getByCode` and throw `CurrencyNotFoundException` on miss.
...
- `AT17`: `upsertSeeded(l10nKey: 'accountType.cash', icon: 'wallet', color: 10, defaultCurrency: USD, sortOrder: 0)` inserts once and returns the row id.
- `AT18`: re-running the same `upsertSeeded(...)` call updates the same logical row, preserves `l10nKey` and any pre-existing `customName` / `isArchived`, and does not duplicate rows.
- `AT19` (formerly Stream B §12 Q2; superseded by Stream C §12 Q5 — `defaultCurrency` is now a required non-null `Currency`): attempting to construct the call without a `defaultCurrency` is a compile-time error, not a runtime one. Dropped as a runtime test case.
- `AT20` (formerly Stream B §12 Q2; superseded): same — `l10nKey` is a required non-null `String`, enforced by the compiler.
...
- `AT22` (Stream C §12 Q5): `upsertSeeded` with a `defaultCurrency` whose `code` is not in `currencies` throws `CurrencyNotFoundException` and writes nothing — the write-path FK pre-check applies to seeded rows identically to user-written rows.
...
|----|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Q1 | ~~`AccountTypeRepository.upsertSeeded` takes discrete keyword args while `save` takes an `AccountType`. Keep the asymmetry, or pass a single `AccountType`?~~ **SUPERSEDED.**                                                                                                | ~~Pass a single `AccountType`.~~ **Superseded by Stream C §12 Q5 (2026-04-22).** The cross-stream review picked Stream C's named-parameter shape as canonical: `Future<int> upsertSeeded({required String l10nKey, required String icon, required int color, required Currency defaultCurrency, required int sortOrder})`. The rationale: seed never surfaces `customName` / `isArchived` / `id`, so a full-model parameter is misleading; named params are the more honest contract at the seed seam. | §1.2 now carries the named-param signature. §6.2 AT17/AT18 call the method with named args; AT19/AT20 are dropped (null `defaultCurrency` / null `l10nKey` are compile-time errors under the new signature); AT22 added for the `CurrencyNotFoundException` case.        |
| Q2 | ~~After Q1, `AccountType.defaultCurrency` is `Currency?` — does the seed path allow null, or must `upsertSeeded` reject it?~~ **SUPERSEDED.**                                                                                                                                | ~~Reject at runtime via `ArgumentError`.~~ **Superseded by Stream C §12 Q5.** Under the named-param signature, `defaultCurrency` is a required non-null `Currency`; the compiler prevents null at call time. Runtime rejection is unnecessary — the former `ArgumentError` path is dead code. `CurrencyNotFoundException` still fires when the non-null `defaultCurrency.code` is not in `currencies` (write-side FK pre-check).                                                                       | §1.2 dartdoc describes the FK pre-check, not the null guard. §6.2 drops AT19 / AT20 and adds AT22.                                                                                                                                                                       |
...
- **Stream C seed call site.** ~~Q1 + Q2 change the `upsertSeeded` signature. Stream C's seed code constructs `AccountType` literals…~~ **Superseded by Stream C §12 Q5 (2026-04-22).** The seed calls `upsertSeeded` with named args directly (`l10nKey:`, `icon:`, `color:`, `defaultCurrency:`, `sortOrder:`) — no `AccountType` literal construction at the seam. Stream C's plan already reflects this.
...
**Inbound from Stream C §12 resolution log (2026-04-22):** Stream C's open-question pass added these obligations on Stream B. Each is already applied in this plan:

- **Stream C §12 Q5 → Stream B §1.2.** `AccountTypeRepository.upsertSeeded` signature flips from `(AccountType type)` to the named-param shape. Applied above; Stream B §12 Q1/Q2 marked superseded.
- **Stream C §12 Q6 → Stream B §1.1 `CurrencyRepository.upsert` consumer contract.** The seed now ships 11 fiats (adds CAD, SGD, AUD, NZD). No API change here — `upsert` is already per-row — but the test harness's currency-seed helper must call `upsert` 11 times, not 7. Stream B's own tests remain fine (§6.1 doesn't assume a row count).
- **Stream C §12 Q7 → Stream B §1.1 + `currencies` schema.** New `CurrencyRepository.updateCustomName(String, String?)` method added above. The `currencies` table gets a `custom_name` nullable column; the v1 Drift snapshot regenerates. Stream B owns:
  - `lib/data/database/tables/currencies_table.dart` — add `TextColumn get customName => text().nullable()();`.
  - `lib/data/models/currency.dart` — add `String? customName` to the Freezed model; `build_runner build` regenerates `currency.freezed.dart`.
  - `drift_schemas/drift_schema_v1.json` — re-dump via `drift_dev schema dump`.
  - `§3.10 CurrencyRepository is MVP read-mostly` — now read-mostly plus one user-rename write; update the "write surface scope" statement.
