...
- (none — all previously deferred questions resolved below)

### Resolved After Initial Planning

- **Undo-window pump duration**: `home_controller.dart:40` exports `const Duration kUndoWindow = Duration(seconds: 4)` as a public top-level constant. Integration tests import and use it directly: `tester.pump(kUndoWindow + const Duration(milliseconds: 100))`. No magic literal needed.
- **`flutter_native_splash` separate YAML**: config is already inline in `pubspec.yaml:85-89` and already points `background_image_android/ios` (plus dark variants) at `assets/splash/sun-splash.png`. The asset exists. Unit 9 scope is narrower than first written: no `pubspec.yaml` changes are needed — only run `dart run flutter_native_splash:create` and verify the regenerated platform files.
- **Exact widget finders for integration tests**: all resolvable from the current source.
  - FAB: `find.byTooltip(l10n.homeFabLabel)` — `home_screen.dart:89` sets `tooltip: l10n.homeFabLabel`.
  - Day-nav chevrons: `find.byTooltip(l10n.homeDayNavPrevLabel)` / `homeDayNavNextLabel` — `day_navigation_header.dart:54,74`.
  - Swipe-delete: `find.text(l10n.commonDelete)` — `SlidableAction` with `label: l10n.commonDelete` at `transaction_tile.dart:97`.
  - Duplicate overflow item: `find.text(l10n.homeDuplicateAction)` — `transaction_tile.dart:163`.
  - `TransactionTile` already wraps swipe-action icons in `Semantics` at lines 161 and 169.
...
**Files:**
- Verify: `assets/splash/sun-splash.png` — asset already exists; confirm it is the final version, not the development placeholder
- No `pubspec.yaml` changes needed — `flutter_native_splash:` config at `pubspec.yaml:85-89` already sets `background_image_android/ios` and their dark variants to `assets/splash/sun-splash.png`
- Regenerated by tooling (not hand-edited): `android/`, `ios/` platform splash files

**Approach:**
- Confirm `assets/splash/sun-splash.png` is the final art asset (not a stand-in); if not, replace with the finalized version before proceeding.
- Confirm the runtime Flutter splash widget in `lib/features/splash/widgets/splash_sun_background.dart` loads `assets/splash/sun-splash.png` so native and Flutter-layer splash reference the same file.
- Run `dart run flutter_native_splash:create` (human / CI step) — no `pubspec.yaml` edits required.
- Verify the native splash on an Android API 29 device, an Android API 33 device (tests the `android_12:` stanza — solid color only, no background image), and an iOS simulator.
- Commit only the regenerated platform files (and the asset replacement if the art changed).
...
| Risk                                                                                                                                                     | Mitigation                                                                                                                                                                                               |
|----------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Integration test for Add-Transaction form requires complex widget interaction (keypad, category picker, save) that may be fragile to widget-tree changes | Use `find.byTooltip` and `find.text` with l10n keys (all widget finders now resolved — see Open Questions above); add `Key(...)` to FAB and Save button only if tooltip-based finders prove insufficient |
| Undo-window timer behavior in integration tests                                                                                                          | Timer constant is `kUndoWindow` (exported from `home_controller.dart`); use `tester.pump(kUndoWindow + const Duration(milliseconds: 100))` to advance past the window deterministically                  |
...
| Android 12+ cannot show a full-screen native background image                                                                                            | Config already accounts for this: `android_12.color`/`color_dark` remain solid-color; only pre-Android-12 and iOS get `background_image_*`. Verify API 33 device shows solid color, not a broken image   |
| 2× text scale tests may reveal layout overflows introduced in M5 that require widget fixes                                                               | Fix overflow at the widget level (add `Flexible`/`FittedBox` or extend the `textScaler` clamp from `SplashDayCount`); do not suppress the test                                                           |
| About screen (webview) cannot be integration-tested without a real network                                                                               | Integration tests skip Settings → About navigation. Tested manually on device.                                                                                                                           |
