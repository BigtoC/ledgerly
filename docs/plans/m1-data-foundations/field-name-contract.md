...
Plus the **enum wire values** (`'expense'`, `'cash'`, …) stored as `TEXT` in the DB.
...
### 3.4 `accounts` — PRD 315–334

| DB column (PRD)                 | Drift getter / Freezed field | Type (domain)         | Notes |
|---------------------------------|------------------------------|-----------------------|-------|
| `id`                            | `id`                         | `int`                 | PRIMARY KEY AUTOINCREMENT. |
| `name`                          | `name`                       | `String`              | User-visible account name. |
| `type`                          | `type`                       | `AccountType` (domain) | Freezed holds enum; Drift holds `String`. Wire values: §4. |
...
No `current_balance` / `tracked_balance` — derived from transactions (PRD 331).

### 3.5 `user_preferences` — PRD 401–414
...
### 4.2 `AccountType` — mirrors `accounts.type` (`TEXT NOT NULL`)

| Dart case | `@JsonValue(...)` | DB-stored `TEXT` |
|-----------|-------------------|------------------|
| `AccountType.cash`  | `@JsonValue('cash')`  | `'cash'`  |
| `AccountType.bank`  | `@JsonValue('bank')`  | `'bank'`  |
| `AccountType.other` | `@JsonValue('other')` | `'other'` |
...
3. **This contract** (`docs/plans/m1-data-foundations/field-name-contract.md`) — row in §3.
...
- **Wire-value rename after first seed.** Once M3 seeds currencies / categories / `Cash` account, the enum wire strings (`'expense'`, `'income'`, `'cash'`, `'bank'`, `'other'`) are **migration-locked** — change only via `schemaVersion` bump + data transform.
...
| `accounts`     | `type`                          | `Account.type` (enum `AccountType`)         |
...
| `transactions` | `memo`, `created_at`, `updated_at` | `Transaction.memo`, `.createdAt`, `.updatedAt` |
...
| FK `REFERENCES currencies(code)` | `transactions.currency`, `accounts.currency` | Fires only with `PRAGMA foreign_keys = ON` (set in `beforeOpen`). M3 repositories must handle FK-violation exceptions. |
...
### 8.5 No CHECK constraints in MVP

Per Stream A §2.3 and §2.4, neither `categories.type` nor `accounts.type` carries a `CHECK (type IN (…))` constraint at the SQL level. Enum validity is enforced in M3 repositories (typed exception on unknown string). Freezed's enum is the static-typing backstop.
...
Disagreements or ambiguities between Stream A and Stream B that require a human decision **before** M3 repository mappers are written. For each, the recommendation is resolved in this doc; the sibling specs reference this resolution.
...
- **Stream B §9 Q1** recommends including on all four for Phase 3 CSV/backup.
- **Stream A** takes no position (no JSON boundary inside Drift).
- **Resolution (this contract):** **Include `fromJson`/`toJson` on all four models** (`Currency`, `Transaction`, `Category`, `Account`). Cheap now; avoids a future migration. Binds the JSON-key naming rule in §2 (camelCase, no `fieldRename`).
...
- **Stream B §9 Q5** hard-codes `'USD'` in the stub.
- **Stream A** does not touch `LocaleService`.
- Not strictly a field-name contract question, but M3's seed reads this value to pick `user_preferences.default_currency`. Flagged here so the seed behaviour is predictable.
- **Resolution (this contract):** **Hard-coded `'USD'` at M1.** M3 replaces with a region-aware lookup; the string the preferences KV receives is still a currency `code` (FK to `currencies.code`), so no rename risk.

### 9.8 Non-contradictions observed (for the record)

Stream A and Stream B **agree** on:

- Drift `@DataClassName` suffix convention (`…Row` except `Currency`) — Stream A §2 header vs. Stream B §8.2 item 3.
- Enum wire values `'expense'` / `'income'` / `'cash'` / `'bank'` / `'other'` — Stream A §9.2 vs. Stream B §3.2 & §3.3.
